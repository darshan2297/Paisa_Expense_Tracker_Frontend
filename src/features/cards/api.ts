...
export async function getCardPayments(): Promise<CardPaymentHistoryItem[]> {
  const response = await apiClient.get<Envelope<CardPaymentHistoryItem[]>>('/cards/payments');
  return response.data.data as CardPaymentHistoryItem[];
}
